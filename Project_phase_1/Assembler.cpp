...
bool Assembler::have_error()
{
	...
	if (st.check_comment())
	{
		cout << "check_comment in have error >>" << st.line << endl;
		return false;
	}
			...
			if (!have_error())
			{
				...
				else
				{
					...
					else
					{
						...
						else if (st.operation.compare("resw") == 0)
							L = 3 * stoi(st.operand);
						else if (st.operation.compare("resb") == 0)
							L = stoi(st.operand);
...
int Assembler::check_operand_format3()
{
	...
	if (st.operand[0] == '#' && !isdigit(st.operand[1]))
		return 1;
	if (st.operand[0] == '@' && !isdigit(st.operand[1]))
		return 2;
	if (isalpha(st.operand[0]))
		return 3;
	if (st.operand[0] == '#' && isdigit(st.operand[1]))
		return 4;
	if (st.operand[0] == '@' && isdigit(st.operand[1]))
		return 5;
	if (isdigit(st.operand[0]))
		return 6;
	if (st.operand.compare("*") == 0)
		return 7;
